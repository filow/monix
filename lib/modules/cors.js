...
class CorsHandler {
  ...
  render(ctx) {
    ...
    if (method === 'OPTIONS') {
      // preflight
      result = u.merge(result, this._renderOrigin(scope, ctx.headers.origin), this._renderMethods(scope), this._renderCredentials(scope), this._renderAllowedHeaders(scope, ctx.headers['access-control-request-headers']), this._renderExposedHeaders(scope), this._renderMaxAge(scope));
    } else {
      result = u.merge(result, this._renderOrigin(scope, ctx.headers.origin), this._renderCredentials(scope), this._renderExposedHeaders(scope));
    }
    return result;
  }
  _middleware() {
