...
class Router {
  ...
  _generateRouteName(method, path) {
    ...
    // 提取[{name: foo}]里面的name
    u.map(this.stack, e => e.name), n => n.startsWith(baseName));
    // 没有可能的重名项目就直接返回
  ...
  middleware() {
    ...
    return function () {
      var ref = _asyncToGenerator(function* (ctx, next) {
        ...
        const action = stack.find(function (item) {
